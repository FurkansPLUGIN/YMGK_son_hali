...
namespace Asama2.DataBase
{
    public class DataFire
    {
		...
		public async Task SaveUserRequest(Stream imgStream1, StorageUser req)
		{
			var ip = GetLocalAddress().Replace(".", "_");

			var postData = await fbClient.Child(ip + "image").PostAsync<StorageUser>(req);
			//var postData2 = await fbClient.Child(ip).PostAsync<StorageUser>(req);

			var imgUrl1 = await new FirebaseStorage("ymgk-69871.appspot.com")

				.Child(ip + "image")
				.Child(postData.Key)
				.PutAsync(imgStream1);

			//var imgUrl2 = await new FirebaseStorage("ymgk-69871.appspot.com")
			...
			var updateData1 = fbClient.Child(ip + "image" + "/" + postData.Key )
								   .PutAsync<StorageUser>(req);
        ...
        {
            var ip = GetLocalAddress().Replace(".", "_");
            var list1 = (await fbClient.Child(ip + "image").OnceAsync<StorageUser>()).Select(item =>
            new StorageUser
            {
        ...
        public async Task deleteImage(string img)
		{
			...
			var delete = (await fbClient.Child(ip + "image").OnceAsync<StorageUser>())
				.FirstOrDefault(a => a.Object.ImageUrl1 == img );
			await fbClient.Child(ip+"image").Child(delete.Key).DeleteAsync();
